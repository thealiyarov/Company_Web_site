...
function Items({ img, itemName, itemDesc }) {
  ...
  const truncatedDesc = showFullDesc ? itemDesc : itemDesc.slice(100, maxLength);
      ...
      <div className="items flex justify-around gap-[70px] h-[250px] items-center w-[62%] rounded-[8px]">
        ...
        <div className="item-right h-[206px] w-[68%]">
          <div className="item-right-container ml-[20px] mt-[20px]">
            <div className="item-right-top w-[95%] text-white text-[36px] text-left">
              <h1 className="cursor-pointer" onClick={toggleShowMore}>
                {itemName}
              </h1>
            </div>
            <div className="item-right-bottom w-[95%] text-[#8E88C1] text-left text-[18px]">
              <p className="itemP">{truncatedDesc}</p>
              {!showFullDesc && (
                <h1 className="read-more cursor-pointer" onClick={toggleShowMore}>
                  {itemDesc}
                </h1>
              )}
            </div>
